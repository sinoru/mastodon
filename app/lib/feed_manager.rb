...
class FeedManager
  ...
  def filter_from_home?(status, receiver_id, crutches, timeline_type = :home)
    ...
    return true  if timeline_type == :home && crutches[:hiding_from_home][status.account_id].present?
  ...
  def build_crutches(receiver_id, statuses)
    ...
    crutches[:hiding_from_home]     = Follow.where(account_id: receiver_id, target_account_id: statuses.map(&:account_id), hide_from_home: true).pluck(:target_account_id).index_with(true)
